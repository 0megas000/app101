...
export function LearnScreen() {
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 860, margin: "0 auto" }}>
      <h2 style={{ fontSize: 28, fontWeight: 900, marginBottom: 6 }}>New to pre-workout?</h2>
      <p style={{ color: "var(--ink-2)", marginBottom: 22 }}>Quick answers to the most common questions. No jargon.</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
        {bootstrap.education.map((t) => (
          <button key={t.id} className="card" style={{ padding: 24, textAlign: "left", cursor: "pointer", display: "flex", flexDirection: "column", gap: 10 }} onClick={() => setOpen(t.slug)}>
            ...
            <span style={{ color: "var(--ink-3)", fontSize: 13 }}>Tap to read →</span>
          </button>
        ))}
      </div>
      {topic && (
        <Modal onClose={() => setOpen(null)}>
          ...
          <h3 style={{ fontSize: 26, fontWeight: 900, marginBottom: 12 }}>{topic.title}</h3>
          <p style={{ color: "var(--ink-2)", fontSize: 16.5, lineHeight: 1.65 }}>{topic.body}</p>
